...
1. **Read `references/_growth-protocol-pointer.md`, then load the shared `radon-skill-growth` skill.** The pointer explains how new learnings flow back into the skill — the in-flow ask template (warm tone, plain English), the Generalization rule (rewrite project-specific findings into universal patterns before committing), the CHANGELOG.md prepend format, and what counts as a "discovery". That shared skill holds the full mechanic. Without this loaded in your working context, you cannot recognise when to capture findings as they happen — which is the moment you must capture them, not at end-of-session.

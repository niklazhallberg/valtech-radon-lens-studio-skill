...
**Architecture + authoring-surface choice (load early, before any scripting)**:
- `references/composition-patterns.md` — 8 recurring lens scene-architecture shapes (multi-pass render-target chain, full-frame effect, script-driven VFX, tracking-driven material, interactive paint, tween-driven animation orchestration, ML/SnapML pipeline, event-graph glue). Each has when-to-use / anatomy / editors involved / pitfalls. Load at Phase 0 when designing the lens architecture — maps the brief to a known shape instead of inventing structure ad-hoc.
- `references/visual-scripting-guide.md` — decision rule for Visual Scripting (node graphs) vs TypeScript components, three handoff mechanisms (Call Script API node, custom-event bus, shared component state), four common patterns (tap→tween→audio, face-tracking trigger, timer loop, math-driven material param), six pitfalls (string-name typos drop silently, profiling blindness, fire-order ambiguity, On Frame vs Late Update, MCP cannot edit nodes). Load when the user shows up with a prototype Visual Script or asks "should I script this or graph it?".
- `references/editor-scripting-api.md` — LS 5.17+ editor-side API surface and when to use `ExecuteEditorCode` (one round trip beats N for any operation touching >3 objects, conditional mutations, computed values, scene audits). Capability comparison vs `scene-graphql`, four practical recipes (bulk rename, missing-component audit, anchor snapshot for diff, material reference scan), six gotchas. Load when probing/mutating at scale via MCP.
- `references/material-editor-guide.md` — when to author custom material graphs vs use built-ins (PBR/Unlit/Sprite), script ↔ shader parameter handoff, six recipes (LUT color grade, screen-space dissolve, UV scroll, face-data-driven distortion, stylized outline, camera-feed blur), seven pitfalls (param-name typos drop silently, Branch nodes don't skip GPU work, shared-by-default instances, onAwake-timing reset), per-material-type node-count budget. Load at Phase 3 polish when a stylized look is in the brief.
- `references/graph-authoring-protocol.md` — four-mode discipline (Discover → Probe → Mutate → Explain) for any work touching Shader / VFX / Script Graph editors, capability-ledger pattern, per-graph priority order (Script Graph first via Custom Node bridge, Shader Graph second via round-trip, VFX last), plugin-author mindset. Load FIRST when a user asks for graph-level edits — the empirical reality is no graph node mutation API exists, this file defines what CC CAN do and how to communicate the boundary to the user.

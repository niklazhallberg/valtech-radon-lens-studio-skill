...
2. **Read `CHANGELOG.md` — at minimum the "Improvements and newly acquired knowledge" section.** This is where every discovery from every prior session has been recorded with a plain-English "what" and "value for user" line. For a colleague who is fresh to this skill (no memory from prior sessions), the CHANGELOG is the replacement for institutional memory — it tells the agent what other Valtech RADON projects have already learned, what traps were sprung in prior builds, and what mitigations exist. **Without reading the CHANGELOG, the agent starts every session ignorant of months of accumulated empirical knowledge** — and is doomed to repeat the same investigations the team has already paid for. Read the last 10-20 entries minimum; if a discovery's file pointer matches a topic that is likely to come up in this project (based on the brief), follow the pointer and read that section of the referenced file too.
